ALTER TABLE public.punishment_spins ALTER COLUMN matchup_id DROP NOT NULL;
ALTER TABLE public.punishment_spins ALTER COLUMN winner_user_id DROP NOT NULL;
...
AS $$
DECLARE
  v_user UUID := auth.uid(); v_week RECORD; v_matchup RECORD; v_option RECORD;
  v_existing public.punishment_spins; v_result public.punishment_spins; v_user_name TEXT;
  v_loser_count INT := 0; v_loser UUID; v_winner UUID; v_low_score NUMERIC; v_high_score NUMERIC;
BEGIN
  ...
  ELSIF v_week.game_format='leaderboard' THEN
    ...
    WITH scores AS (
      SELECT lm.user_id,COALESCE(ws.total_points,0)::NUMERIC AS total_points FROM public.league_members lm
      LEFT JOIN public.weekly_scores ws ON ws.week_id=_week_id AND ws.user_id=lm.user_id WHERE lm.league_id=v_week.league_id
    ) SELECT MIN(total_points),MAX(total_points) INTO v_low_score,v_high_score FROM scores;
    WITH scores AS (
      SELECT lm.user_id,COALESCE(ws.total_points,0)::NUMERIC AS total_points FROM public.league_members lm
      LEFT JOIN public.weekly_scores ws ON ws.week_id=_week_id AND ws.user_id=lm.user_id WHERE lm.league_id=v_week.league_id
    ) SELECT COUNT(*),MIN(user_id) INTO v_loser_count,v_loser FROM scores WHERE total_points=v_low_score;
    ...
    WITH scores AS (
      SELECT lm.user_id,COALESCE(ws.total_points,0)::NUMERIC AS total_points FROM public.league_members lm
      LEFT JOIN public.weekly_scores ws ON ws.week_id=_week_id AND ws.user_id=lm.user_id WHERE lm.league_id=v_week.league_id
    ) SELECT MIN(user_id) INTO v_winner FROM scores WHERE total_points=v_high_score;
  ELSE
    RAISE EXCEPTION 'Unsupported league format';
  END IF;

  SELECT * INTO v_option FROM public.punishment_options po
  WHERE po.is_active=TRUE AND (po.league_id IS NULL OR po.league_id=v_week.league_id) ORDER BY random() LIMIT 1;
  ...
  VALUES(v_week.league_id,_week_id,CASE WHEN v_week.game_format='head_to_head' THEN v_matchup.id ELSE NULL END,v_loser,v_winner,v_option.id,v_option.label,v_option.description,v_option.emoji)
...
CREATE OR REPLACE FUNCTION public.remove_league_punishment(_punishment_id UUID)
RETURNS VOID LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE v_option public.punishment_options;
BEGIN
  SELECT * INTO v_option FROM public.punishment_options WHERE id=_punishment_id;
  IF NOT FOUND THEN RAISE EXCEPTION 'Punishment not found'; END IF;
  IF v_option.league_id IS NULL THEN RAISE EXCEPTION 'Default punishments cannot be removed'; END IF;
  IF NOT public.is_league_admin(auth.uid(),v_option.league_id) THEN RAISE EXCEPTION 'Only a league owner or admin can remove custom punishments'; END IF;
  UPDATE public.punishment_options SET is_active=FALSE WHERE id=_punishment_id;
END;
$$;
...
REVOKE ALL ON FUNCTION public.remove_league_punishment(UUID) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.remove_league_punishment(UUID) TO authenticated;
